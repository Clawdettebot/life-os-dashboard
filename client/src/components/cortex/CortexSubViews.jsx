        ...
        <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="flex-1 flex flex-col gap-6 h-full overflow-hidden">
            ...
            <div className="flex-1 flex gap-6 overflow-hidden">
                <motion.div variants={staggerItem} className="w-1/3 flex flex-col gap-4 border-r border-[var(--border-color)] pr-6 h-full overflow-hidden">
                    ...
                    <LobsterScrollArea className="flex-1" contentClassName="space-y-3 pb-4">
                        {entries.map((item, i) => {
                            const isActive = selectedEntry === item;
                            return (
                                <div key={i} onClick={() => setSelectedEntry(item)} className={`p-5 rounded-[1.5rem] border cursor-pointer transition-all duration-300 relative overflow-hidden group ${isActive ? 'bg-yellow-500/5 border-yellow-500/50 shadow-[0_0_20px_rgba(234,179,8,0.1)]' : 'bg-[var(--bg-card)] border-[var(--border-color)] hover:border-[var(--border-highlight)]'}`}>
                                    <div className="flex justify-between items-center mb-3">
                                        <span className={`text-[9px] font-space-mono uppercase tracking-widest flex items-center gap-1.5 font-bold ${isActive ? 'text-yellow-500' : 'text-yellow-500/70'}`}><Lightbulb size={10} /> {item.category || 'IDEA'}</span>
                                        <span className="text-[9px] font-space-mono text-[var(--text-faint)]">{new Date(item.created_at).toLocaleDateString()}</span>
                                    </div>
                                    <h4 className={`text-sm font-bold font-space-grotesk leading-snug mb-2 line-clamp-2 ${isActive ? 'text-yellow-400' : 'text-[var(--text-main)]'}`}>{item.title}</h4>
                ...
                <motion.div variants={staggerItem} className="flex-1 bg-[var(--bg-card)] border border-[var(--border-color)] rounded-[2.5rem] p-8 flex flex-col relative overflow-hidden hover-spotlight group transition-colors h-full">
                    ...
                    <div className="flex items-center gap-4 mb-8 pb-6 border-b border-[var(--border-color)] relative z-10 shrink-0">
                        ...
                        <h2 className="text-lg font-bold text-[var(--text-main)] font-space-grotesk leading-snug">
                            {selectedEntry ? selectedEntry.title : 'Select an entry to view details'}
                        </h2>
                    </div>

                    <LobsterScrollArea className="flex-1" contentClassName="font-space-mono text-[11px] text-[var(--text-muted)] leading-relaxed space-y-6 relative z-10 pb-4">
                        <AnimatePresence mode="wait">
                            {selectedEntry ? (
                                <motion.div
                                    ...
                                    className="space-y-6"
                                >
                                    <div className="flex items-center gap-2 mb-2 text-yellow-500 font-bold uppercase tracking-widest text-[9px]"><Sparkles size={12} /> Data Decrypted</div>
                                    {selectedEntry.media_url && (
                                        <img src={selectedEntry.media_url} alt="entry media" className="w-full max-h-64 object-cover rounded-2xl border border-[var(--border-color)]" />
                                    )}
                                    <div className="text-[var(--text-main)] text-sm font-space-mono leading-relaxed whitespace-pre-wrap">
                                        {selectedEntry.content}
                                    </div>
                                    <div className="pt-6 mt-6 border-t border-[var(--border-color)] flex justify-between items-center opacity-60">
                                        <span className="text-[9px] uppercase tracking-widest">Entry Log: {new Date(selectedEntry.created_at).toLocaleString()}</span>
                                    </div>
                                </motion.div>
                            ) : (
                                <motion.div
                                    ...
                                    animate={{ opacity: 1 }}
                                    exit={{ opacity: 0 }}
                                    className="bg-[var(--bg-panel)] border border-[var(--border-color)] p-4 rounded-xl text-[var(--text-main)]"
                                >
...
export const CortexHitchhikersGuide = ({ entries }) => {
    ...
    return (
        <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="flex-1 flex flex-col gap-6 h-full overflow-hidden relative">
            ...
            <LobsterScrollArea className="flex-1" contentClassName="pb-10">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    {entries.map((item, i) => {
                        const Icon = [Tent, Navigation, Droplet, Compass, Leaf, Settings][i % 6];
                        return (
                            <motion.div variants={staggerItem} key={i} onClick={() => setSelectedEntry(item)} className="group relative overflow-hidden rounded-[2.5rem] border border-[var(--border-color)] bg-[var(--bg-card)] flex flex-col hover:border-cyan-500/50 hover:shadow-[0_0_30px_rgba(34,211,238,0.1)] transition-all duration-500 hover-spotlight h-80 cursor-pointer">
            ...
            <AnimatePresence>
                {selectedEntry && (
                    <motion.div
                        ...
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
                        className="absolute inset-0 z-50 flex items-center justify-center p-6 bg-[var(--bg-base)]/80 backdrop-blur-md rounded-[2.5rem]"
                    ...
                    >
                        <motion.div
                            ...
                            className="w-full max-w-4xl max-h-[90%] bg-[var(--bg-card)] border border-cyan-500/30 rounded-[2.5rem] overflow-hidden shadow-[0_0_50px_rgba(34,211,238,0.15)] relative flex flex-col"
                        >
                            <div className="absolute inset-0 bg-tech-grid opacity-10 pointer-events-none" />

                            <div className="relative z-10 p-6 border-b border-[var(--border-color)] flex justify-between items-center bg-cyan-900/10 backdrop-blur-sm">
                                <div className="flex items-center gap-4">
                                    <div className="w-10 h-10 rounded-full border border-cyan-500/30 flex items-center justify-center text-cyan-400 bg-[var(--bg-base)]">
                                        <Book size={18} />
                                    </div>
                                    <div>
                                        <h3 className="text-xl font-bold font-space-grotesk text-[var(--text-main)] uppercase tracking-widest">{selectedEntry.title}</h3>
                                        <span className="text-cyan-400 font-space-mono font-bold text-[10px] uppercase tracking-widest">
                                            {selectedEntry.category || 'SKILL'}
                                        </span>
                                    </div>
                                </div>
                                <button onClick={() => setSelectedEntry(null)} className="p-2 rounded-full text-[var(--text-muted)] hover:text-cyan-400 hover:bg-cyan-500/10 transition-colors">
                                    <X size={20} />
                                </button>
                            </div>

                            <LobsterScrollArea className="flex-1 relative z-10 bg-[var(--bg-card)]" contentClassName="p-8">
                                {selectedEntry.media_url && (
                                    <div className="w-full h-64 md:h-96 rounded-2xl overflow-hidden mb-8 border border-[var(--border-color)] relative">
                                        <div className="absolute inset-0 bg-cyan-500/10 mix-blend-overlay z-10 pointer-events-none" />
                                        <img src={selectedEntry.media_url} alt={selectedEntry.title} className="w-full h-full object-cover" />
                                    </div>
                                )}
                                <div className="font-space-mono text-sm leading-relaxed text-[var(--text-main)] whitespace-pre-wrap">
                                    {selectedEntry.content}
                                </div>
                            </LobsterScrollArea>
                        </motion.div>
    ...
    return (
        <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="flex-1 flex flex-col gap-6 h-full overflow-hidden relative">
            ...
            <LobsterScrollArea className="flex-1" contentClassName="pb-10">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    {entries.map((item, i) => (
                        <motion.div variants={staggerItem} key={i} onClick={() => setSelectedEntry(item)} className="group relative overflow-hidden rounded-[2.5rem] border border-[var(--border-color)] bg-[var(--bg-card)] flex flex-col hover:border-orange-500/50 hover:shadow-[0_0_30px_rgba(249,115,22,0.1)] transition-all duration-500 hover-spotlight h-80 cursor-pointer">
            ...
            <AnimatePresence>
                {selectedEntry && (
                    <motion.div
                        ...
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
                        className="absolute inset-0 z-50 flex justify-end bg-[var(--bg-base)]/50 backdrop-blur-sm rounded-[2.5rem] overflow-hidden"
                        onClick={() => setSelectedEntry(null)}
                    >
                        <motion.div
                            initial={{ x: '100%' }}
                            ...
                            className="w-full max-w-lg h-full bg-[var(--bg-card)] border-l border-orange-500/30 shadow-[-20px_0_50px_rgba(0,0,0,0.5)] flex flex-col relative"
                            onClick={(e) => e.stopPropagation()}
                        >
                            {selectedEntry.media_url ? (
                                <div className="h-64 relative w-full border-b border-[var(--border-color)] shrink-0">
                                    <img src={selectedEntry.media_url} alt={selectedEntry.title} className="w-full h-full object-cover" />
                                    <div className="absolute inset-0 bg-gradient-to-t from-[var(--bg-card)] to-transparent" />
                                    <button onClick={() => setSelectedEntry(null)} className="absolute top-6 right-6 w-10 h-10 rounded-full bg-black/50 backdrop-blur-md border border-[var(--border-color)] flex items-center justify-center text-white hover:bg-orange-500/50 transition-colors z-10">
                                        <X size={20} />
                                    </button>
                                </div>
                            ) : (
                                <div className="h-40 bg-[var(--bg-panel)] relative w-full border-b border-[var(--border-color)] shrink-0 flex items-center justify-center">
                                    <Utensils size={48} className="text-orange-500/20" />
                                    <button onClick={() => setSelectedEntry(null)} className="absolute top-6 right-6 w-10 h-10 rounded-full bg-[var(--bg-card)] border border-[var(--border-color)] flex items-center justify-center text-[var(--text-muted)] hover:text-orange-400 transition-colors z-10">
                                        <X size={20} />
                                    </button>
                                </div>
                            )}

                            <LobsterScrollArea className="flex-1" contentClassName="p-8">
                                <span className="inline-block px-3 py-1 bg-orange-500/10 border border-orange-500/30 text-orange-400 rounded-full text-[10px] font-space-mono font-bold uppercase tracking-widest mb-4">
                                    {selectedEntry.category || 'RECIPE'}
                                </span>
                                <h2 className="text-2xl font-bold font-space-grotesk text-[var(--text-main)] mb-6 leading-tight">{selectedEntry.title}</h2>

                                <div className="space-y-6 text-[var(--text-muted)] font-space-mono text-sm leading-relaxed whitespace-pre-wrap">
                                    {selectedEntry.content}
                                </div>
                            </LobsterScrollArea>
                        </motion.div>
...
export const CortexEmeraldTablets = ({ entries }) => {
    ...
    return (
        <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="flex-1 flex flex-col h-full overflow-hidden relative">
            ...
            <LobsterScrollArea direction="horizontal" className="flex-1 relative w-full" contentClassName="py-12 flex items-center">
                ...
                <div className="flex gap-12 min-w-max px-8 items-center h-full relative z-10">
                    {entries.map((item, i) => (
                        <div key={i} className={`relative flex flex-col items-center w-80 ${i % 2 === 0 ? 'translate-y-[calc(-50%-2rem)]' : 'translate-y-[calc(50%+2rem)]'}`}>
                            ...
                            <motion.div onClick={() => setSelectedEntry(item)} variants={staggerItem} className="w-full bg-[var(--bg-card)] border border-[var(--border-color)] rounded-[2rem] p-6 hover-spotlight group hover:border-emerald-500/50 hover:shadow-[0_0_30px_rgba(16,185,129,0.1)] transition-all duration-500 cursor-pointer">
                ...
                {selectedEntry && (
                    ...
                    >
                        <motion.div
                            ...
                            onClick={(e) => e.stopPropagation()}
                            className="w-full max-w-3xl max-h-[90%] bg-[var(--bg-card)] border border-emerald-500/30 rounded-[2.5rem] overflow-hidden shadow-[0_0_50px_rgba(16,185,129,0.15)] relative flex flex-col"
                        >
                            <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-emerald-600 via-emerald-400 to-emerald-600" />

                            <div className="p-8 border-b border-[var(--border-color)] flex justify-between items-start bg-emerald-900/10">
                                <div>
                                    <div className="flex items-center gap-2 mb-3 text-[10px] font-space-mono text-emerald-400 font-bold tracking-widest uppercase">
                                        <History size={14} /> TEMPORAL ARCHIVE ENTRY
                                    </div>
                                    <h3 className="text-2xl font-bold font-space-grotesk text-[var(--text-main)]">{selectedEntry.title}</h3>
                                    <div className="flex items-center gap-2 mt-4 text-[11px] font-space-mono text-[var(--text-muted)]">
                                        <Clock size={12} /> Logged: {new Date(selectedEntry.created_at).toLocaleDateString()}
                                    </div>
                                </div>
                                <button onClick={() => setSelectedEntry(null)} className="w-10 h-10 rounded-full bg-[var(--bg-base)] border border-[var(--border-color)] flex items-center justify-center text-[var(--text-muted)] hover:text-emerald-400 hover:border-emerald-500/50 transition-colors shrink-0">
                                    <X size={20} />
                                </button>
                            </div>

                            <LobsterScrollArea className="flex-1 bg-[var(--bg-base)]" contentClassName="p-8">
                                {selectedEntry.media_url && (
                                    <div className="w-full rounded-2xl overflow-hidden mb-8 border border-[var(--border-color)]">
                                        <img src={selectedEntry.media_url} alt={selectedEntry.title} className="w-full h-auto object-contain max-h-[50vh]" />
                                    </div>
                                )}
                                <div className="font-space-mono text-[13px] leading-loose text-[var(--text-main)] whitespace-pre-wrap">
                                    {selectedEntry.content}
                                </div>
                            </LobsterScrollArea>
                        </motion.div>
